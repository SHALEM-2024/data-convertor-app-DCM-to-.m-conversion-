...
# ---------- Header detectors (includes KENNLINIE + FESTWERT) ----------
...
def _parse_single_block(block_text: str):
    ...
    Returns:
      ...
      - z : np.ndarray (2D for data; 1D for KENNLINIE; 0-D scalar for FESTWERT)
      - df: DataFrame
          * 2D: rows=Y, cols=X
    ...
    while i < len(lines):
        ...
        if line.startswith("WERT"):
            ...
            if kind == "KENNLINIE":
                ...
                # For 2D data, values are read inside ST/Y loop
                pass
            continue
...
def _matlab_ident(name: str) -> str:
    """Make a safe MATLAB identifier (for variable names)."""
...
def _fmt_vec(arr: np.ndarray, ndp: int | None = None) -> str:
    ...
    def fmt(v):
        return f"{v:.{ndp}f}" if ndp is not None else f"{v:.10g}"
...
def build_matlab_m_bytes(
    ...
    # ✅ These match your MATLAB naming convention:
    curve_value_suffix: str = "Y",  # KENNLINIE values -> <base>_Y
    scalar_value_suffix: str = "V",  # FESTWERT scalar -> <base>_V
    z_suffix: str = "Z",  # 2D table -> <base>_Z
    # Optional per-map overrides, e.g. {"KFRLSN": {"Z": "XZ"}}
    per_map_suffix_override: Optional[Dict[str, Dict[str, str]]] = None,
) -> bytes:
    """
    Build a MATLAB .m script that defines variables in your desired pattern:

    2D maps (KENNFELD/GRUPPENKENNFELD):
      <base>_X, <base>_Y, <base>_<Zsuffix>   (default Zsuffix='Z')

    1D curves (KENNLINIE):
      <base>_X, <base>_<curve_value_suffix>  (default curve_value_suffix='Y')

    Scalars (FESTWERT):
      <base>_<scalar_value_suffix>           (default scalar_value_suffix='V')

    alias_map: maps DCM name -> MATLAB base name (WITHOUT _X/_Y/_Z/_V)
      Example: {"KFMIOP": "M_KFMIOP"}

    per_map_suffix_override: maps DCM name -> overrides like:
      {"KFRLSN": {"Z": "XZ"}}  => emits <base>_XZ instead of <base>_Z
    """
    b = _io.StringIO()
    ...
    def uniq(var_name: str) -> str:
        """Make MATLAB-safe identifier AND ensure it is unique in this file."""
        base = _matlab_ident(var_name)
        if base not in used_vars:
            used_vars.add(base)
            return base
        k = 2
        while f"{base}_{k}" in used_vars:
            k += 1
        out = f"{base}_{k}"
        used_vars.add(out)
        return out
    ...
    for orig_name, (x, y, z, df, meta) in data.items():
        ...
        # DCM name -> MATLAB base name (no suffix)
        base_alias = (alias_map or {}).get(orig_name, orig_name) or orig_name
        base_alias = base_alias.strip()

        # Per-map overrides (optional)
        overrides = (per_map_suffix_override or {}).get(orig_name, {})
        z_suf = overrides.get("Z", z_suffix)
        curve_suf = overrides.get("CURVE", curve_value_suffix)
        scalar_suf = overrides.get("SCALAR", scalar_value_suffix)

        # ---------------- FESTWERT (scalar) ----------------
        if dim == 0:
            vname = uniq(f"{base_alias}_{scalar_suf}")
            val = float(np.asarray(z).reshape(-1)[0])
            b.write(f"% ---- {base_alias} (FESTWERT; from {orig_name}) ----\n")
            b.write(f"{vname} = {_fmt_scalar(val, round_digits)};\n\n")
            continue

        # ---------------- Common: X ----------------
        xname = uniq(f"{base_alias}_X")
        b.write(f"% ---- {base_alias} (dim={dim}; from {orig_name}) ----\n")
        b.write(f"{xname} = {_fmt_vec(np.asarray(x), round_digits)};\n")

        # ---------------- KENNLINIE (1D) ----------------
        if dim == 1:
            yname = uniq(f"{base_alias}_{curve_suf}")
            b.write(f"{yname} = {_fmt_vec(np.asarray(z), round_digits)};\n\n")
            continue

        # ---------------- 2D map ----------------
        yname = uniq(f"{base_alias}_Y")
        zname = uniq(f"{base_alias}_{z_suf}")
        b.write(f"{yname} = {_fmt_vec(np.asarray(y), round_digits)};\n")
        b.write(f"{zname} = {_fmt_mat(np.asarray(z), round_digits)};\n\n")
...
def load_parse_build_m(
    ...
    round_digits: int | None = None,
    alias_map: Optional[Dict[str, str]] = None,
    curve_value_suffix: str = "Y",
    scalar_value_suffix: str = "V",
    z_suffix: str = "Z",
    per_map_suffix_override: Optional[Dict[str, Dict[str, str]]] = None,
    ...
    m_bytes = build_matlab_m_bytes(
        ...
        alias_map=alias_map,
        curve_value_suffix=curve_value_suffix,
        scalar_value_suffix=scalar_value_suffix,
        z_suffix=z_suffix,
